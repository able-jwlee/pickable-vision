...
5. 모양 게이트    둥글기(면적÷최소외접원) ≥ 0.55 · solidity ≥ 0.75 · 크기 창
...
`min_t` 가 감도 노브이고, 낮추면 흐린 콜로니까지 잡히면서 오검출이 는다.

**평균이 아니라 중앙값·MAD 를 쓴다.** 평균을 쓰면 바깥 링에 섞인 소수의 밝은
픽셀 — 긁힘·메니스커스·응결선·인쇄 글씨 획 — 이 배경 추정을 끌어당겨,
**균일한 한천 위에 거짓 콜로니가 생긴다.** 이웃 콜로니가 링에 걸쳐도 같은 이유로
흔들리지 않는다.

**산포에는 하한을 둔다** (`BLOB_NOISE_FLOOR = 0.5`). 완전히 균일한 영역은 분산이
0 이라 위 식의 분모가 0 으로 가서 **t 가 발산한다** — 과노출로 클리핑된 부분이나
합성 이미지에서 구조가 없는 곳에 거대 blob 이 잡히는 실패가 이것이다. 8비트
센서가 갖는 최소 양자화 노이즈 ±0.5 LSB 를 바닥으로 깐다. 통계로 판정하기로 한
이상 선택이 아니라 필수 방어다.

### 5단계 — 모양 게이트

**주된 판정은 둥글기**(`min_roundness`, 면적 ÷ 최소외접원 면적)다. 둘레 기반
원형도(`min_circularity`)는 **기각돼 기본 0** 이다 — 경계 거칠기에 극도로
민감해서, 합집합 후보의 이진화 성분(임계값에서 잘려 둘레가 거칠다)을 부당하게
버렸다. 끄자 같은 감도에서 재현율 74.4 → 75.7%.

면적 기반은 거칠기에 둔감하면서 얇고 긴 획(글씨)은 여전히 걸러낸다 — 외접원
대비 면적이 작기 때문이다.

**둥글기 완화는 권하지 않는다.** 같은 정밀도(82.8%)에서 이 값을 0.45 로 푸는
것보다 감도를 내리는 쪽이 더 많이 맞힌다(74.7% 대 72.5%). 모양 게이트를 푸는
건 곡선을 올리는 게 아니라 곡선 위를 나쁜 방향으로 미끄러지는 것이다.
