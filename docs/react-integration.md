...
### 최근 계약 변경 (2026-08-27, 1.4.0)

| 변경 | 영향 |
|---|---|
| **`Colony` 에 `diam_frac` 추가** | 크기 축 히스토그램을 이 값으로 바꿀 것. `radius × 2 ÷ plate_size_ref` 는 게이트 값과 다르다 — 이 문서가 잘못 안내했다 (§11.1) |
...
```js
// 이렇게 베껴두면 서버만 고쳤을 때 조용히 어긋난다. 하지 말 것.
...
> 목업 두 개에 있던 이 복제는 **제거했다.** `operator-ui.html` 은 응답의
> `applied_params.min_t` 를 표시하고, 감도를 바꾸면 "검출 후 표시됩니다" 로
> 되돌린다. 정밀도·재현율 힌트는 **감도값 자체**를 키로 삼아(43/46/50/81 이
> 실측 앵커다) 변환이 아예 필요 없게 했다.
...
| `plate_size_ref` | 크기 창(`min/max_diam_frac`)의 **분모**, 원본 px. "5% = 몇 px" 환산에 쓴다. 히스토그램은 이제 `Colony.diam_frac` 을 쓸 것 (§11.1) |
...
1. **크기 축에 검출 지름 히스토그램**을 그리고 크기 창을 띠로 겹친다.
   `colonies` 의 **`diam_frac`** 분포를 그대로 그리면 된다 (1.4.0 신규).
   오퍼레이터가 숫자를 추측하지 않고 분포를 보고 자를 수 있다.

   > **`radius × 2 ÷ plate_size_ref` 로 계산하지 말 것.** 이 문서가 1.3.0
   > 까지 그렇게 안내했는데 **틀렸다.** `radius` 는 표시용이라 크기 게이트가
   > 본 값에 보정(`radius_mode`, `radius_scale`)이 붙어 있고, 그 배수가
   > 검출마다 다르다 — 실측 39장·2175건에서 중앙 1.39, p95 2.16, 최대
   > 7.73. 상수가 아니라 나눠서 되돌릴 수 없다. 그래서 히스토그램이 크기
   > 창 슬라이더보다 오른쪽으로 밀려 보였고, 오퍼레이터가 창을 슬라이더
   > 위치와 다르게 인식했다. `diam_frac` 은 게이트가 **실제로 비교한** 값이라
   > 슬라이더와 같은 축에 놓인다.
   >
   > 그래서 `diam_frac` 은 화면의 원보다 작다. 눈으로 잰 지름이 아니다.
   > 마커 크기는 계속 `radius` 를 쓸 것 — 둘은 용도가 다르다.
