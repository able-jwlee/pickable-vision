...
> `polarity` 는 여기 있었으나 전문가 등급으로 재분류했다 — [§2](#2-알고리즘-파라미터--고급-접힘) 참고.
...
| 파라미터 | 기본값 | 효과 | 실측 |
|---|---|---|---|
| `polarity` | `"auto"` | 콜로니가 한천보다 밝은지/어두운지 자동 판정 | 39/39 100% 정확 — 그래도 **UI 에 노출 안 함** ↓ |
...
**`polarity` — 전문가 등급, UI 에 노출 안 함.** 자동 판정(`"auto"`)은 39장 전부
정확하다(39/39). 그런데도 노출하지 않는 것은 **틀렸을 때의 낙폭** 때문이다 —
실측 `971.jpg` 에서 극성이 뒤집히면 검출이 98개 → 3개로 붕괴하는데, 오퍼레이터가
화면에서 이 실패를 "밝기 극성" 탓으로 되짚을 방법이 없다. 판정 마진이 애매한
경우는 자동으로 양극성(`both`) 검출로 되돌아가므로 그 자체로 보호돼 있다.
`polarity: "both"` 는 API 에는 남겨둔다 — 새 카메라·새 배지에서 자동 판정이
처음 틀렸을 때 개발자·지원팀이 쓰는 되돌림 경로다.
