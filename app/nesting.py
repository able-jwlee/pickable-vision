...
후처리로 분리한 이유는, 검출 경로를 건드리지 않으면 옵션을 끈 상태에서 **검출
결과(좌표·반지름 등)가 바이트 단위로 동일**하다는 것을 보장할 수 있기
때문이다. 다만 **응답 자체는 그대로가 아니다** — 모든 `Colony` 가 이제
`parent_id` 를 갖고 `applied_params` 에는 `has_chroma` 가 추가돼, 옵션을
꺼도 응답 스키마는 이전보다 늘어난다(additive). 엄격한 스키마 검증을 하는
클라이언트는 이 점을 감안할 것.
